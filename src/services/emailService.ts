...
export interface FreeWebsiteOfferData {
  fullName: string;
  email: string;
  phone: string;
  address: string;
...
// Send free website offer email
export const sendFreeWebsiteOfferEmail = async (data: FreeWebsiteOfferData): Promise<{ success: boolean; error?: any }> => {
  try {
    const templateParams = {
      to_email: EMAILJS_CONFIG.RECIPIENT_EMAIL,
      ...
      social_media_profile: data.socialMediaProfile,
      short_description: data.shortDescription,
      professional_summary: data.professionalSummary,
      timestamp: new Date().toISOString(),
      subject: 'New Free Website Request'
    };

    const result = await emailjs.send(
      EMAILJS_CONFIG.SERVICE_ID,
      EMAILJS_CONFIG.TEMPLATES.FREE_WEBSITE_OFFER,
      templateParams,
      EMAILJS_CONFIG.PUBLIC_KEY
    );

    console.log('Free website offer email sent successfully:', result);
    return { success: true };
  } catch (error) {
    console.error('Error sending free website offer email:', error);
    return { success: false, error };
  }
};
